...
def run_cmd_v2(cmd, task_id, step_num, total_steps, sub_status_msg, cwd=None, env=None, track_dir=None, target_total=0):
    db = SessionLocal(); t = db.query(Task).filter(Task.id == task_id).first()
    if t: 
        t.sub_status, t.current_step, t.progress = sub_status_msg, step_num, 0
        db.commit()
    db.close()
    
    # 物理判断：如果目标文件已足够，直接跳过调用
    if track_dir and target_total > 0 and os.path.exists(track_dir):
        ...
        if count >= target_total:
            print(f"[{task_id}] Step {step_num} already done ({count}/{target_total}). Skipping.")
    ...
    print(f"\n" + "="*60 + f"\n🚀 [TASK {task_id}] Step {step_num}: {sub_status_msg}\n💻 Command: {' '.join(cmd)}\n" + "="*60 + "\n")
...
def run_v2_pipeline(task_id, resume_from_step=1):
    ...
    try:
        if resume_from_step <= 1: run_cmd_v2(["python", "v2_generate_positives.py", "--config", config_path], task_id, 1, total_steps, "生成正样本", scripts_dir, env, track_dir=pos_train_dir, target_total=n_pos)
        if resume_from_step <= 2: run_cmd_v2(["python", "v2_generate_similars.py", "--config", config_path], task_id, 2, total_steps, "生成近似词样本", scripts_dir, env, track_dir=neg_train_dir, target_total=n_neg)
