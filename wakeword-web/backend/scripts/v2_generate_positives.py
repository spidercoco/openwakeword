...
def generate_samples(text, max_samples, output_dir, batch_size=1, model=None, overwrite=False):
...
if __name__ == "__main__":
    ...
    generate_samples(config["target_phrase"], config["n_samples"], train_dir, config.get("tts_batch_size", 1), model, overwrite=args.overwrite)
    
    print(f"Generating testing positives...")
    generate_samples(config["target_phrase"], config["n_samples_val"], test_dir, config.get("tts_batch_size", 1), model, overwrite=args.overwrite)
